...
/*   Updated: 2020/05/01 14:42:40 by smaccary         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		forward_handler(t_vars *vars)
{
	...
	{
		if (vars->map.worldMap[(int)(vars->cam.x + vars->cam.dir_x * SPEED)]
			[(int)(vars->cam.y)] - '0' == 0)
			vars->cam.x += vars->cam.dir_x * SPEED;
		if (vars->map.worldMap[(int)(vars->cam.x)]
			[(int)(vars->cam.y + vars->cam.dir_y * SPEED)] - '0' == 0)
...
static int		right_handler(t_vars *vars)
{
	...
	if (key_chr(vars->keys, RIGHT_KEY, K_BUFF_SIZE))
	{
		...
		vars->cam.dir_x = vars->cam.dir_x * cos(-TURN_SPEED)
			- vars->cam.dir_y * sin(-TURN_SPEED);
		vars->cam.dir_y = old_dir_x * sin(-TURN_SPEED)
			+ vars->cam.dir_y * cos(-TURN_SPEED);
		old_plane_x = vars->cam.plane.x;
		vars->cam.plane.x = vars->cam.plane.x * cos(-TURN_SPEED)
			- vars->cam.plane.y * sin(-TURN_SPEED);
		vars->cam.plane.y = old_plane_x * sin(-TURN_SPEED)
			+ vars->cam.plane.y * cos(-TURN_SPEED);
		return (1);
	}
	return (0);
}
